...
rightInfo.addEventListener('click', async (e) => {
    ...
    if (target.classList.contains('edit-btn')) {
        ...
        const newText = prompt("멀로 바꿀건데:", currentText);
...
async function appendComment(user, text, id,isMyComment,liked,likeCount,createdAt = null) {
    ...
    const actionButtons = isMyComment 
        ? `<div class="comment-actions" style="font-size:12px; color:gray; margin-left:35px; margin-top:10px; margin-bottom:10px;">
    ...
    commentDiv.innerHTML = `
        ...
        <p class="comment-text" style="margin: 0 0 5px 35px;">${text}</p>
        ${actionButtons}
    `;
